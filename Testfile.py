...
app.layout = html.Div([
    ...
    dbc.Row( dbc.Col(dcc.Graph(id = 'speelveld',figure =  fig_table_speelveld),
                     ),
             ),
    dbc.Row(dbc.Col(html.H1("Simulation results with the given settings", style={'text-align':'center'}),
                     ),
             ),
    dbc.Row([dbc.Col(dcc.Graph(id = 'orders met',figure =  fig_order_deadlines_met),
                    width = { 'offset': 2, 'order': 2},
                    ),
            dbc.Col(dcc.Graph(id='priority orders met', figure=figorderdeadlinesmet_priority),
